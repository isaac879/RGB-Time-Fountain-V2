...
void switchCase(char command){
    ...
    switch(command){
        ...
        break;
        case 'p'://set pump frequency
            setDropFrequency(stringText);
        break;
        case 'P'://set pump PWM
            setDropPWM(stringText);
        break;
        case 'm'://set mode
        ...
        break;
        case '+'://Bluetooth module sends some chars when connecting, begging with a + so just ignore it and flush the serial buffer
            delay(50);
            serialFlush();
        break;
...
int setDropFrequency(char freq[]){
    if(!((atof(freq) >= DROP_FREQUENCY_MIN &&  atof(freq) <= DROP_FREQUENCY_MAX) || atof(freq) == 0)){//60 is the actual maximum pump frequency
        printi(F("Invalid duty... It must be between 50Hz and 60Hz. Frequency entered: "), atof(freq), 3, F("Hz\n"));
        return -1;
    }
...
int setDropPWM(char pwm[]){
    if(!(atof(pwm) >= 0 &&  atof(pwm) <= 255)){
        printi(F("Invalid duty... It must be between 0 and 255. Frequency entered: "), atof(pwm), 3, F("\n"));
        return -1;
    }
    if(pwm[0] == '\0'){//if it's an emty string
        pwm[0] = '0';//set to char 0
    }
    ...
    int returnVal = Wire.endTransmission();
    if(returnVal == 0){//Sucsessful
        pump_drop_frequency = atof(pwm); 
        printi(F("Pump PWM set to: "));
        printi(pwm);
        printi(F("\n"));
        return 0;
    }
